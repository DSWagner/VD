...
const GroupedBarChart = ({ tableData, directionColors }) => {
  useEffect(() => {
    ...
    const labeledColors = labels.map((label, index) => [
      label,
    ...
    const processData = () => {
      ...
      function calculateAverage(dataArray) {
        return dataArray.map((element) => {
          ...
          const mean =
            element.length > 0 ? (sum / element.length).toFixed(2) : "0.00";
          return mean;
        });
      }
      ...
      return {
        ...
        datasets: [
          ...
          },
          {
            label: "Avg Fix",
            data: avgFixAvg,
            backgroundColor: "rgba(133,235,217, 0.5)",
          },
          {
            label: "Max Fix",
            data: maxFixAvg,
    ...
    const updateColors = () => {
      ...
      tickLabels.forEach((label) => {
        ...
        const foundPair = labeledColors.find(
          (pair) => pair[0] === labelWithoutDegree
        );
        if (foundPair) {
